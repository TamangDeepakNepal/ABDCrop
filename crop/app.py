...
def farmer_dashboard():
    ...
    # Get only this farmer's prediction history
    predictions = Recommendation.query.filter_by(
        farmer_id=session["user_id"]
    ).order_by(
        Recommendation.prediction_time.desc()
    ).all()
...
@app.route("/admin/delete-farmer/<int:id>")
...
def predict_page():
    # Must be logged in to access the prediction page
    if "user_id" not in session:
        return redirect(url_for("login"))
...
def predict():
    # Must be logged in to run a prediction
    if "user_id" not in session:
        return jsonify({'error': 'Please log in to get a crop recommendation.', 'redirect': url_for('login')}), 401
    ...
    try:
        ...
        # Save prediction to database if a farmer is logged in
        ...
        soil_score, soil_tips = get_soil_health(
            vals[0], vals[1], vals[2], vals[5]
        )
